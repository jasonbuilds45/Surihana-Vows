...
export default function GalleryError({ error, reset }: GalleryErrorProps) {
  useEffect(() => {
    console.error("[surihana] Gallery error:", error);
  }, [error]);
    ...
    <div className="flex min-h-[65vh] items-center justify-center px-6">
      <div className="max-w-md space-y-6 text-center">
        ...
        <div className="space-y-2">
          <h2 className="font-display text-3xl text-stone-900">Gallery could not be loaded</h2>
          <p className="text-sm leading-7 text-stone-500">
            The photo gallery is temporarily unavailable. The images are safe — please try again
            in a moment.
          </p>
          {error.digest ? (
